...
Your response (5-10 words only):`,R=await ea(t.id,e.id,M);if(R.success&&R.prompt){let w=R.prompt.trim();w=w.replace(/^["']|["']$/g,"").trim();const W=w.split(/[.!?\n]/)[0].trim(),K=W.length>80?W.slice(0,77)+"...":W;y({description:K})}else alert("Failed to generate description: "+(R.error||"Unknown error"))}catch(M){alert("Error generating description: "+M.message)}finally{f(!1)}}return o.jsxs("div",{className:"agent-editor",children:[o.jsx("style",{children:`
        .agent-editor {
          background: var(--bg-secondary);
          border: 1px solid var(--border-color);
          border-radius: var(--radius-lg);
