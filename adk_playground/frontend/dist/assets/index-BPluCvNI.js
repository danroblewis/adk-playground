...
def my_callback(tool: BaseTool, tool_args: Dict[str, Any], tool_context: ToolContext, result: Dict) -> Optional[Dict]:
    ...
    return None
`;default:return l1("before_agent")}}function fE({onSelectCallback:e}){var C,F;const{project:t,updateProject:n,addCustomCallback:r,updateCustomCallback:i,removeCustomCallback:s}=ht(),[a,l]=k.useState(""),[c,u]=k.useState(null),[d,p]=k.useState(null),[f,h]=k.useState(!1),g=k.useRef(null);if(!t)return null;const v=t.custom_callbacks||[],w=v.find(N=>N.id===c);function m(N){u(N),e==null||e(N)}k.useEffect(()=>{w?(l(w.code),p(null)):l("")},[c,w]);function x(){const N=pE(),M="new_callback",R={id:N,name:M,description:"",module_path:`callbacks.${M}`,code:l1("before_agent"),state_keys_used:[]};r(R),m(N)}function y(N,M){M.stopPropagation(),confirm("Delete this callback?")&&(s(N),c===N&&(e==null||e(null),u(null),l("")))}function j(){if(!c)return;const N=v.find(V=>V.id===c);if(!N)return;const M=N.name.trim();if(!M){alert("Please enter a name");return}if(!Em(M)){p("Name must contain only alphanumeric characters and underscores");return}if(v.find(V=>V.name===M&&V.id!==N.id)){p("A callback with this name already exists");return}i(N.id,{code:a,name:M,description:N.description,module_path:`callbacks.${M}`,state_keys_used:N.state_keys_used}),p(null)}function T(N,M){g.current=M;try{a1(M,{endpoint:"/api/code-completion",language:"python"})}catch(R){console.warn("Failed to register Monacopilot completion:",R)}}async function S(){if(w){h(!0);try{let N="before_agent";const M=w.name.toLowerCase(),R=w.description.toLowerCase();M.includes("after_agent")||R.includes("after agent")?N="after_agent":M.includes("before_model")||R.includes("before model")?N="before_model":M.includes("after_model")||R.includes("after model")?N="after_model":M.includes("before_tool")||R.includes("before tool")?N="before_tool":M.includes("after_tool")||R.includes("after tool")?N="after_tool":(M.includes("before_agent")||R.includes("before agent"))&&(N="before_agent");const V=await Jy(t.id,w.name,w.description,N,w.state_keys_used);V.success&&V.code?(l(V.code),i(w.id,{code:V.code})):(console.error("Failed to generate callback code:",V.error),alert("Failed to generate callback code: "+(V.error||"Unknown error")))}catch(N){console.error("Error generating callback code:",N),alert("Error generating callback code: "+N.message)}finally{h(!1)}}}const P=((F=(C=t.app)==null?void 0:C.state_keys)==null?void 0:F.map(N=>N.name))||[];return o.jsxs("div",{className:"tools-panel",children:[o.jsx("style",{children:`
        .tools-panel {
          display: flex;
          height: 100%;
          background: var(--bg-primary);
...
`),isError:t.isError===!0}:{text:typeof t=="string"?t:JSON.stringify(t,null,2),isError:!1}}function rg(e,t){if(!t||!t.trim())return e;const n=t.trim();let r=e;try{r=JSON.parse(e)}catch{}if(n.startsWith("js:")){const i=n.slice(3).trim();try{const a=new Function("value","data",`return ${i}`)(e,r);return typeof a=="string"?a:JSON.stringify(a,null,2)}catch(s){return`[JS error: ${s}]`}}if(n.startsWith("."))try{const i=n.slice(1).split(".").filter(Boolean);let s=r;for(const a of i){const l=a.match(/^(\w+)?\[(\d+)\]$/);l?(l[1]&&(s=s[l[1]]),s=s[parseInt(l[2])]):s=s[a]}return s==null?"[No match]":typeof s=="string"?s:JSON.stringify(s,null,2)}catch(i){return`[Path error: ${i.message}]`}try{const s=new Function("value","data",`return ${n}`)(e,r);return typeof s=="string"?s:JSON.stringify(s,null,2)}catch(i){return`[Transform error: ${i.message}]`}}function I8({project:e,selectedEventIndex:t,sandboxMode:n}){var oe,ue;const{watches:r,updateWatch:i,addWatch:s,removeWatch:a,runEvents:l}=ht(),[c,u]=k.useState(!1),[d,p]=k.useState(null),[f,h]=k.useState({}),[g,v]=k.useState(new Set),[w,m]=k.useState(""),[x,y]=k.useState(""),[j,T]=k.useState({}),[S,P]=k.useState(""),[C,F]=k.useState([]),[N,M]=k.useState(null),[R,V]=k.useState(null),[A,O]=k.useState(!1);k.useEffect(()=>{Vd().then(F).catch(console.error)},[]);const D=k.useMemo(()=>{const J=e.mcp_servers||[],B=new Set(J.map(X=>X.name)),G=C.filter(X=>!B.has(X.name));return[...J,...G]},[e.mcp_servers,C]),E=k.useCallback(async J=>{if(!(f[J]||g.has(J))){v(B=>new Set([...B,J]));try{const B=await Se(`/projects/${e.id}/mcp-servers/${encodeURIComponent(J)}/test-connection`,{method:"POST"});B.success&&h(G=>({...G,[J]:B.tools}))}catch(B){console.error("Failed to load tools:",B)}finally{v(B=>{const G=new Set(B);return G.delete(J),G})}}},[e.id,f,g]);k.useEffect(()=>{var X;if(d)return;if(!w||!x){T({});return}const B=(f[w]||[]).find(Q=>Q.name===x);if(!((X=B==null?void 0:B.parameters)!=null&&X.properties)){T({});return}const G={};Object.entries(B.parameters.properties).forEach(([Q,ae])=>{ae.type==="string"?G[Q]=ae.default||"":ae.type==="number"||ae.type==="integer"?G[Q]=ae.default||0:ae.type==="boolean"?G[Q]=ae.default||!1:G[Q]=ae.default||null}),T(G)},[w,x,f,d]);const $=()=>{p(null),m(""),y(""),T({}),P(""),M(null),V(null),u(!0)},I=J=>{if(p(J.id),m(J.serverName),y(J.toolName),T({...J.args}),P(J.transform||""),J.result){const{text:B}=wc(J.result);M(B),V(null)}else M(null),V(null);f[J.serverName]||E(J.serverName),u(!0)},_=async()=>{var B;if(!w||!x)return;O(!0),V(null);const J=((B=e.app)==null?void 0:B.id)||`app_${e.id}`;try{const G=await Se(`/projects/${e.id}/run-mcp-tool`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({server_name:w,tool_name:x,arguments:j,sandbox_mode:n,app_id:n?J:void 0})}),{text:X,isError:Q}=wc(G);Q?(V(X),M(null)):(M(X),V(null))}catch(G){V(String(G)),M(null)}finally{O(!1)}},W=k.useMemo(()=>N?!S||!S.trim()?N:rg(N,S):null,[N,S]),K=()=>{if(!(!w||!x)){if(d){i(d,{serverName:w,toolName:x,args:{...j},transform:S||void 0});const J=r.find(B=>B.id===d);J&&z({...J,serverName:w,toolName:x,args:j,transform:S||void 0,history:J.history||[]})}else{const J={id:`watch-${Date.now()}`,serverName:w,toolName:x,args:{...j},transform:S||void 0,history:[]};s(J),z(J)}u(!1)}},b=J=>{a(J)},z=k.useCallback(async(J,B)=>{var ae;i(J.id,{isLoading:!0,error:void 0});const G=B??l.length-1,X=Date.now(),Q=((ae=e.app)==null?void 0:ae.id)||`app_${e.id}`;try{const ge=await Se(`/projects/${e.id}/run-mcp-tool`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({server_name:J.serverName,tool_name:J.toolName,arguments:J.args,sandbox_mode:n,app_id:n?Q:void 0})}),ke={eventIndex:G,timestamp:X,result:ge},ee=[...J.history||[],ke];i(J.id,{result:ge,isLoading:!1,lastRun:X,history:ee})}catch(ge){const ke={eventIndex:G,timestamp:X,error:String(ge)},ee=[...J.history||[],ke];i(J.id,{error:String(ge),isLoading:!1,lastRun:X,history:ee})}},[e.id,(oe=e.app)==null?void 0:oe.id,i,l.length,n]),L=()=>{r.forEach(J=>z(J))},H=k.useMemo(()=>!w||!x?null:(f[w]||[]).find(B=>B.name===x),[w,x,f]);return o.jsxs("div",{className:"tool-watch-panel",children:[o.jsxs("div",{className:"watch-header",children:[o.jsx(Or,{size:14}),o.jsx("span",{children:"Tool Watch"}),o.jsx("span",{className:"watch-auto-badge",title:"Watches auto-refresh on every event",children:"⚡ Auto"}),o.jsxs("div",{className:"watch-actions",children:[o.jsx("button",{className:"watch-btn",onClick:L,title:"Refresh all",children:o.jsx(Qt,{size:12})}),o.jsx("button",{className:"watch-btn",onClick:$,title:"Add watch",children:o.jsx(Qe,{size:12})})]})]}),r.length===0?o.jsxs("div",{className:"watch-empty",children:[o.jsx(Or,{size:20,style:{opacity:.3}}),o.jsx("span",{children:"No watch expressions"}),o.jsxs("button",{className:"add-watch-btn",onClick:$,children:[o.jsx(Qe,{size:12})," Add Tool Watch"]})]}):o.jsx("div",{className:"watch-list",children:r.map(J=>{let B=J.result,G=J.error;if(t!==null&&J.history&&J.history.length>0){const ke=J.history.filter(ee=>ee.eventIndex<=t);if(ke.length>0){const ee=ke[ke.length-1];B=ee.result,G=ee.error}else B=void 0,G=void 0}const{text:X,isError:Q}=B?wc(B):{text:"",isError:!1},ae=B?rg(X,J.transform):"",ge=G||Q;return o.jsxs("div",{className:`watch-item ${ge?"error":""}`,children:[o.jsxs("div",{className:"watch-item-header",children:[o.jsxs("span",{className:"watch-expr",children:[o.jsx("span",{className:"watch-server",children:J.serverName}),o.jsx("span",{className:"watch-tool",children:J.toolName}),Object.keys(J.args).length>0&&o.jsxs("span",{className:"watch-args",children:["(",Object.entries(J.args).map(([ke,ee])=>`${ke}=${JSON.stringify(ee)}`).join(", "),")"]}),t!==null&&o.jsxs("span",{className:"watch-time-indicator",children:["@",t]})]}),o.jsxs("div",{className:"watch-item-actions",children:[o.jsx("button",{onClick:()=>I(J),title:"Edit watch",children:o.jsx(un,{size:10})}),o.jsx("button",{onClick:()=>z(J),title:"Refresh",children:J.isLoading?o.jsx(Qt,{size:10,className:"spin"}):o.jsx(Qt,{size:10})}),o.jsx("button",{onClick:()=>b(J.id),title:"Remove",children:o.jsx(Be,{size:10})})]})]}),o.jsx("div",{className:"watch-result",children:J.isLoading?o.jsx("span",{className:"loading",children:"Loading..."}):G?o.jsx("span",{className:"error",children:G}):B?o.jsx("pre",{className:Q?"error-text":"",children:ae}):o.jsx("span",{className:"no-result",children:t!==null?"No data at this event":"Not yet run"})})]},J.id)})}),c&&o.jsx("div",{className:"watch-dialog-overlay",onClick:()=>u(!1),children:o.jsxs("div",{className:"watch-dialog",onClick:J=>J.stopPropagation(),children:[o.jsxs("div",{className:"dialog-header",children:[o.jsx("span",{children:d?"Edit Watch":"Add Tool Watch"}),o.jsx("button",{onClick:()=>u(!1),children:o.jsx(Ai,{size:14})})]}),o.jsxs("div",{className:"dialog-body",children:[o.jsxs("div",{className:"form-row",children:[o.jsx("label",{children:"MCP Server"}),o.jsxs("select",{value:w,onChange:J=>{m(J.target.value),d||y(""),J.target.value&&E(J.target.value)},children:[o.jsx("option",{value:"",children:"Select server..."}),D.map(J=>o.jsx("option",{value:J.name,children:J.name},J.name))]})]}),o.jsxs("div",{className:"form-row",children:[o.jsx("label",{children:"Tool"}),o.jsxs("select",{value:x,onChange:J=>y(J.target.value),disabled:!w||g.has(w),children:[o.jsx("option",{value:"",children:g.has(w)?"Loading tools...":"Select tool..."}),(f[w]||[]).map(J=>o.jsx("option",{value:J.name,children:J.name},J.name))]})]}),(H==null?void 0:H.description)&&o.jsx("div",{className:"tool-desc",children:H.description}),((ue=H==null?void 0:H.parameters)==null?void 0:ue.properties)&&Object.keys(H.parameters.properties).length>0&&o.jsxs("div",{className:"tool-args",children:[o.jsx("label",{children:"Arguments"}),Object.entries(H.parameters.properties).map(([J,B])=>{var G,X;return o.jsxs("div",{className:"arg-row",children:[o.jsxs("span",{className:"arg-name",children:[J,((G=H.parameters.required)==null?void 0:G.includes(J))&&o.jsx("span",{className:"required",children:"*"})]}),o.jsx("input",{type:B.type==="number"||B.type==="integer"?"number":"text",value:typeof j[J]=="object"?JSON.stringify(j[J]):j[J]??"",onChange:Q=>T(ae=>({...ae,[J]:Q.target.value})),placeholder:((X=B.description)==null?void 0:X.slice(0,40))||J})]},J)})]}),w&&x&&o.jsxs("div",{className:"test-section",children:[o.jsxs("div",{className:"test-header",children:[o.jsx("label",{children:"Test & Preview"}),o.jsxs("button",{className:"test-btn",onClick:_,disabled:A,children:[A?o.jsx(Qt,{size:12,className:"spin"}):o.jsx(pr,{size:12}),A?"Running...":"Test Run"]})]}),R&&o.jsxs("div",{className:"test-result error",children:[o.jsx("span",{className:"test-label",children:"Error:"}),o.jsx("pre",{children:R})]}),N&&o.jsxs("div",{className:"test-result",children:[o.jsx("span",{className:"test-label",children:"Raw Result:"}),o.jsx("pre",{children:N})]})]}),o.jsxs("div",{className:"form-row transform-row",children:[o.jsx("label",{children:"Transform (optional)"}),o.jsx("input",{type:"text",value:S,onChange:J=>P(J.target.value),placeholder:"e.g., .items[0].name or .content[].text"}),o.jsxs("div",{className:"transform-hints",children:[o.jsx("span",{className:"hint-title",children:"Path:"}),o.jsx("code",{onClick:()=>P(".items[0].name"),children:".items[0].name"}),o.jsx("code",{onClick:()=>P(".content[0].text"),children:".content[0].text"}),o.jsx("code",{onClick:()=>P(".result.data"),children:".result.data"}),o.jsx("span",{className:"hint-title",children:"JS:"}),o.jsx("code",{onClick:()=>P("js:value.split('\\n')[0]"),children:"js:value.split('\\n')[0]"}),o.jsx("code",{onClick:()=>P("js:data.items?.length"),children:"js:data.items?.length"})]})]}),N&&S&&o.jsxs("div",{className:"transform-preview",children:[o.jsx("span",{className:"test-label",children:"Transform Preview:"}),o.jsx("pre",{className:W!=null&&W.startsWith("[Transform error")?"error":"",children:W})]})]}),o.jsxs("div",{className:"dialog-footer",children:[o.jsx("button",{className:"cancel-btn",onClick:()=>u(!1),children:"Cancel"}),o.jsx("button",{className:"add-btn",onClick:K,disabled:!w||!x,children:d?"Save Changes":"Add Watch"})]})]})})]})}function O8(){var Pp,Lp;const{project:e,updateProject:t,isRunning:n,setIsRunning:r,runEvents:i,addRunEvent:s,clearRunEvents:a,clearWatchHistories:l,runAgentId:c,setRunAgentId:u,watches:d,updateWatch:p,currentSessionId:f,setCurrentSessionId:h}=ht(),[g,v]=k.useState(""),[w,m]=k.useState(null),[x,y]=k.useState(null),[j,T]=k.useState(null),[S,P]=k.useState(""),[C,F]=k.useState(new Set(["agent_start","agent_end","tool_call","tool_result","model_call","model_response","state_change","callback_start","callback_end","callback_error"])),[N,M]=k.useState(()=>{const U=localStorage.getItem("sandboxMode");return U!==null?U==="true":!0});k.useEffect(()=>{localStorage.setItem("sandboxMode",String(N))},[N]);const[R,V]=k.useState(null),[A,O]=k.useState(!1),[D,E]=k.useState({}),[$,I]=k.useState(!1),[_,W]=k.useState("agent"),K=k.useRef(null),b=k.useRef(!0),z=k.useRef(0),[L,H]=k.useState(!0),[oe,ue]=k.useState(!0),[J,B]=k.useState(!1),[G,X]=k.useState(!1),[Q,ae]=k.useState(360),[ge,ke]=k.useState(!1),[ee,ce]=k.useState(!1),[be,se]=k.useState(!1),[Ce,q]=k.useState([]),Z=k.useRef(null);k.useEffect(()=>{const U=localStorage.getItem("promptHistory");if(U)try{const te=JSON.parse(U),de=Object.entries(te).map(([me,pe])=>({prompt:me,count:pe})).sort((me,pe)=>pe.count-me.count);q(de)}catch(te){console.error("Failed to parse prompt history:",te)}},[]);const le=k.useCallback(U=>{const te=U.trim();if(!te)return;const de=localStorage.getItem("promptHistory"),me=de?JSON.parse(de):{};me[te]=(me[te]||0)+1,localStorage.setItem("promptHistory",JSON.stringify(me));const pe=Object.entries(me).map(([Ae,Le])=>({prompt:Ae,count:Le})).sort((Ae,Le)=>Le.count-Ae.count);q(pe)},[]),xe=k.useMemo(()=>{const U=g.toLowerCase().trim();return Ce.filter(te=>!U||te.prompt.toLowerCase().includes(U)).slice(0,10)},[Ce,g]),[Y,re]=k.useState([60,80,100,80,1]),[ye,ve]=k.useState(null),_e=k.useRef(0),Ue=k.useRef(0),[ct,nt]=k.useState([]),[Ee,rt]=k.useState(null),[qe,Cn]=k.useState(!1),Ht=k.useRef(0),Sp=k.useCallback(async(U,te)=>{var Ae;if(!e)return;p(U.id,{isLoading:!0,error:void 0});const de=te??i.length-1,me=Date.now(),pe=((Ae=e.app)==null?void 0:Ae.id)||`app_${e.id}`;try{const Le=await Se(`/projects/${e.id}/run-mcp-tool`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({server_name:U.serverName,tool_name:U.toolName,arguments:U.args,sandbox_mode:N,app_id:N?pe:void 0})}),Nn={eventIndex:de,timestamp:me,result:Le},fn=[...U.history||[],Nn];p(U.id,{result:Le,isLoading:!1,lastRun:me,history:fn})}catch(Le){const Nn={eventIndex:de,timestamp:me,error:String(Le)},fn=[...U.history||[],Nn];p(U.id,{error:String(Le),isLoading:!1,lastRun:me,history:fn})}},[e==null?void 0:e.id,(Pp=e==null?void 0:e.app)==null?void 0:Pp.id,p,i.length,N]);k.useEffect(()=>{if(i.length>Ht.current&&d.length>0){const U=i.length-1;d.forEach(te=>{te.isLoading||Sp(te,U)})}Ht.current=i.length},[i.length,d,Sp]),k.useEffect(()=>{c!==null&&(Cp(c),u(null))},[c,u]);const[Mi,Cp]=k.useState(null),ls=k.useRef(null),wl=k.useRef(null);k.useEffect(()=>{if(!ge)return;const U=de=>{if(!wl.current)return;const pe=wl.current.getBoundingClientRect().right-de.clientX;ae(Math.min(600,Math.max(200,pe)))},te=()=>{ke(!1)};return document.addEventListener("mousemove",U),document.addEventListener("mouseup",te),()=>{document.removeEventListener("mousemove",U),document.removeEventListener("mouseup",te)}},[ge]),k.useEffect(()=>{if(ye===null)return;const U=de=>{const me=de.clientX-_e.current,pe=Math.max(40,Ue.current+me);re(Ae=>{const Le=[...Ae];return Le[ye]=pe,Le})},te=()=>{ve(null)};return document.addEventListener("mousemove",U),document.addEventListener("mouseup",te),document.body.style.cursor="col-resize",document.body.style.userSelect="none",()=>{document.removeEventListener("mousemove",U),document.removeEventListener("mouseup",te),document.body.style.cursor="",document.body.style.userSelect=""}},[ye]);const cs=(U,te)=>{te.preventDefault(),_e.current=te.clientX,Ue.current=Y[U],ve(U)},Np=Y.map((U,te)=>te===Y.length-1?`minmax(${U}px, 1fr)`:`${U}px`).join(" "),O1=k.useMemo(()=>i.length===0?{min:0,max:0}:{min:i[0].timestamp,max:i[i.length-1].timestamp},[i]),Pt=k.useMemo(()=>i.filter((U,te)=>{var de,me,pe;if(j&&(U.timestamp<j[0]||U.timestamp>j[1])||C.size>0&&!C.has(U.event_type))return!1;if(L&&U.event_type==="model_response"){const Ae=((me=(de=U.data)==null?void 0:de.response_content)==null?void 0:me.parts)||((pe=U.data)==null?void 0:pe.parts)||[],Le=Ae.some(fn=>fn.type==="function_call"),Nn=Ae.some(fn=>fn.type==="text");if(!Le&&!Nn)return!1}return!(S&&!JSON.stringify(U).toLowerCase().includes(S.toLowerCase()))}),[i,j,C,S,L]),us=k.useMemo(()=>{var me;let U=0,te=0;const de=x!==null?x+1:i.length;for(let pe=0;pe<de;pe++){const Ae=i[pe];Ae.event_type==="model_response"&&((me=Ae.data)!=null&&me.token_counts)&&(U+=Ae.data.token_counts.input||0,te+=Ae.data.token_counts.output||0)}return{input:U,output:te,total:U+te}},[i,x]),Ep=x!==null?i[x]:null;k.useEffect(()=>{if(!e){nt([]);return}(async()=>{Cn(!0);try{const te=await vu(e.id);nt(te)}catch(te){console.error("Failed to load sessions:",te),nt([])}finally{Cn(!1)}})()},[e]);const kl=k.useCallback(async U=>{if(!e){rt(null);return}if(!U){a(),l(),h(null),rt(null),y(null),T(null);return}try{const te=await Dy(e.id,U);a(),l(),h(te.id),rt(U),y(null),T(null);for(const de of te.events)s(de)}catch(te){alert(`Failed to load session: ${te.message||"Unknown error"}`)}},[e,a,l,h,s]);k.useEffect(()=>{if(!e||ct.length===0||qe)return;const te=new URLSearchParams(window.location.search).get("session");if(te)if(ct.some(me=>me.id===te)){kl(te);const me=window.location.pathname;window.history.replaceState({},"",me)}else console.warn(`Session ${te} not found in available sessions`)},[e,ct,qe,kl]),k.useEffect(()=>{n&&ls.current&&(ls.current.scrollTop=ls.current.scrollHeight)},[i.length,n]);const Ri=k.useCallback(U=>{const te=U??g;if(!e||!te.trim()||n)return;ce(!0),le(te),se(!1),v(te),w&&(w.close(),m(null)),f||(a(),l(),rt(null)),r(!0),y(null),T(null);const de=Wy(e.id);m(de),de.onopen=()=>{de.send(JSON.stringify({message:te,agent_id:Mi||void 0,session_id:f||void 0,sandbox_mode:N}))},de.onmessage=me=>{var Ae;const pe=JSON.parse(me.data);if(pe.event_type==="agent_start"&&((Ae=pe.data)!=null&&Ae.session_id)){const Le=pe.data.session_id;pe.data.session_reused,h(Le),Le&&ct.some(Nn=>Nn.id===Le)&&rt(Le)}else if(pe.type==="session_started")h(pe.session_id),pe.session_id&&ct.some(Le=>Le.id===pe.session_id)&&rt(pe.session_id);else if(pe.type==="sandbox_starting")s({timestamp:Date.now()/1e3,event_type:"agent_start",agent_name:"sandbox",data:{message:"Starting Docker sandbox..."}});else if(pe.type==="sandbox_started")s({timestamp:Date.now()/1e3,event_type:"agent_start",agent_name:"sandbox",data:{message:`Sandbox started (ID: ${pe.sandbox_id})`}});else if(pe.type==="sandbox_response")s({timestamp:Date.now()/1e3,event_type:"model_response",agent_name:"sandbox",data:pe.data});else if(pe.event_type==="approval_required"||pe.type==="network_request"&&pe.event_type==="approval_required"){const Le={id:pe.id,method:pe.method||"GET",url:pe.url,host:pe.host,source:pe.source||"agent",headers:pe.headers||{},timeout:pe.timeout||30};V(Le),s({timestamp:Date.now()/1e3,event_type:"callback_start",agent_name:"sandbox",data:{callback_name:"network_approval",callback_type:"approval",message:`⚠️ Network request to ${pe.host} requires approval`,host:pe.host,url:pe.url}})}else pe.type==="completed"?(r(!1),de.close()):pe.type==="error"?(r(!1),s({timestamp:Date.now()/1e3,event_type:"agent_end",agent_name:"system",data:{error:pe.error}})):s(pe)},de.onerror=()=>{r(!1),s({timestamp:Date.now()/1e3,event_type:"agent_end",agent_name:"system",data:{error:"WebSocket connection error"}})},de.onclose=()=>{r(!1)}},[e,g,n,w,a,r,s,Mi,f,N,le]),$1=k.useCallback(()=>{w==null||w.close(),r(!1)},[w,r]),D1=k.useCallback(async(U,te,de)=>{var fn,Mp,Rp,Ip,Op,$p,Dp,Fp,Bp,Up,Vp,Wp,Hp,qp,Jp,Kp,Gp,Yp,Qp,Xp,Zp,ef,tf,nf,rf,of;if(!R||!e)return;const me=((fn=e.app)==null?void 0:fn.id)||e.id,pe=U?"allow_pattern":"allow_once",Ae=U||R.host,Le=te||"exact",Nn=de?`/sandbox/${me}/approval?project_id=${e.id}`:`/sandbox/${me}/approval`;try{if(await Se(Nn,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({request_id:R.id,action:pe,pattern:Ae,pattern_type:Le,persist:de||!1})}),de&&pe==="allow_pattern"){const jl=((Ip=(Rp=(Mp=e.app)==null?void 0:Mp.sandbox)==null?void 0:Rp.allowlist)==null?void 0:Ip.user)||[],X1={id:`pattern_${Date.now().toString(36)}`,pattern:Ae,pattern_type:Le,source:"approved",added_at:new Date().toISOString()};t({app:{...e.app,sandbox:{...(Op=e.app)==null?void 0:Op.sandbox,enabled:((Dp=($p=e.app)==null?void 0:$p.sandbox)==null?void 0:Dp.enabled)??!1,allow_all_network:((Bp=(Fp=e.app)==null?void 0:Fp.sandbox)==null?void 0:Bp.allow_all_network)??!1,allowlist:{auto:((Wp=(Vp=(Up=e.app)==null?void 0:Up.sandbox)==null?void 0:Vp.allowlist)==null?void 0:Wp.auto)||[],user:[...jl,X1]},unknown_action:((qp=(Hp=e.app)==null?void 0:Hp.sandbox)==null?void 0:qp.unknown_action)??"ask",approval_timeout:((Kp=(Jp=e.app)==null?void 0:Jp.sandbox)==null?void 0:Kp.approval_timeout)??30,agent_memory_limit_mb:((Yp=(Gp=e.app)==null?void 0:Gp.sandbox)==null?void 0:Yp.agent_memory_limit_mb)??512,agent_cpu_limit:((Xp=(Qp=e.app)==null?void 0:Qp.sandbox)==null?void 0:Xp.agent_cpu_limit)??1,mcp_memory_limit_mb:((ef=(Zp=e.app)==null?void 0:Zp.sandbox)==null?void 0:ef.mcp_memory_limit_mb)??256,mcp_cpu_limit:((nf=(tf=e.app)==null?void 0:tf.sandbox)==null?void 0:nf.mcp_cpu_limit)??.5,run_timeout:((of=(rf=e.app)==null?void 0:rf.sandbox)==null?void 0:of.run_timeout)??300}}})}s({timestamp:Date.now()/1e3,event_type:"callback_end",agent_name:"sandbox",data:{callback_name:"network_approval",callback_type:"approval",message:`✅ Approved: ${Ae}`,pattern:Ae,action:pe}})}catch(jl){console.error("Failed to approve:",jl)}V(null)},[R,e,s,t]),F1=k.useCallback(async()=>{var te;if(!R||!e)return;const U=((te=e.app)==null?void 0:te.id)||e.id;try{await Se(`/sandbox/${U}/approval`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({request_id:R.id,action:"deny"})}),s({timestamp:Date.now()/1e3,event_type:"callback_end",agent_name:"sandbox",data:{callback_name:"network_approval",callback_type:"approval",message:`❌ Denied: ${R.host}`,host:R.host,action:"deny"}})}catch(de){console.error("Failed to deny:",de)}V(null)},[R,e,s]),Ii=k.useCallback(async(U=!0)=>{var de;if(!e)return;const te=((de=e.app)==null?void 0:de.id)||`app_${e.id}`;U&&I(!0);try{const[me,pe]=await Promise.all([Se(`/sandbox/${te}/logs?container=agent&tail=500`).catch(()=>null),Se(`/sandbox/${te}/logs?container=gateway&tail=500`).catch(()=>null)]);E({agent:(me==null?void 0:me.logs)||(me==null?void 0:me.error)||"No logs available",gateway:(pe==null?void 0:pe.logs)||(pe==null?void 0:pe.error)||"No logs available"})}catch(me){console.error("Failed to fetch logs:",me),E({agent:`Error fetching logs: ${me}`,gateway:`Error fetching logs: ${me}`})}finally{U&&I(!1)}},[e]),B1=k.useCallback(()=>{O(!0),Ii()},[Ii]);k.useEffect(()=>{if(!A||!K.current||$)return;const U=K.current,te=z.current,de=U.scrollHeight;setTimeout(()=>{if(K.current){if(b.current)K.current.scrollTop=K.current.scrollHeight;else if(te>0&&de>te){const me=de-te;K.current.scrollTop+=me}z.current=K.current.scrollHeight}},50)},[A,_,D,$]),k.useEffect(()=>{A&&(b.current=!0,z.current=0)},[A,_]);const U1=k.useCallback(()=>{if(!K.current)return;const U=K.current,de=U.scrollHeight-U.scrollTop-U.clientHeight<50;b.current=de},[]);k.useEffect(()=>{if(!A)return;const U=setInterval(()=>{Ii(!1)},3e3);return()=>clearInterval(U)},[A,Ii]),k.useEffect(()=>{const U=te=>{if((te.metaKey||te.ctrlKey)&&te.key==="Enter"){te.preventDefault(),Ri();return}if(te.key==="ArrowDown"||te.key==="ArrowUp"){if(te.target instanceof HTMLInputElement||te.target instanceof HTMLTextAreaElement||(te.preventDefault(),Pt.length===0))return;if(te.key==="ArrowDown")if(x===null){const de=i.indexOf(Pt[0]);y(de)}else{const de=Pt.findIndex(me=>i.indexOf(me)===x);if(de<Pt.length-1){const me=i.indexOf(Pt[de+1]);y(me)}}else if(te.key==="ArrowUp")if(x===null){const de=i.indexOf(Pt[Pt.length-1]);y(de)}else{const de=Pt.findIndex(me=>i.indexOf(me)===x);if(de>0){const me=i.indexOf(Pt[de-1]);y(me)}}}};return window.addEventListener("keydown",U),()=>window.removeEventListener("keydown",U)},[Ri,Pt,x,i]);const V1=k.useCallback(()=>{var pe;if(i.length===0)return;const U={version:1,exportedAt:new Date().toISOString(),projectId:e==null?void 0:e.id,projectName:e==null?void 0:e.name,agentId:Mi||((pe=e==null?void 0:e.app)==null?void 0:pe.root_agent_id),events:i},te=new Blob([JSON.stringify(U,null,2)],{type:"application/json"}),de=URL.createObjectURL(te),me=document.createElement("a");me.href=de,me.download=`run-${(e==null?void 0:e.name)||"export"}-${new Date().toISOString().slice(0,19).replace(/:/g,"-")}.json`,document.body.appendChild(me),me.click(),document.body.removeChild(me),URL.revokeObjectURL(de)},[i,e,Mi]),W1=k.useCallback(async()=>{if(!f||!e){alert("No active session to save");return}try{const U=await $y(f);if(U.success){alert(U.message||"Session saved to memory successfully");try{const te=await vu(e.id);nt(te)}catch{}}else alert(`Failed to save to memory: ${U.error||"Unknown error"}`)}catch(U){alert(`Error saving to memory: ${U.message||"Unknown error"}`)}},[f,e]),[H1,ds]=k.useState(!1),[q1,J1]=k.useState([]),[Oi,zp]=k.useState(""),[_l,K1]=k.useState("Test Case from Session"),[Tp,Ap]=k.useState(!1),G1=k.useCallback(async()=>{if(!f||!e){alert("No active session to create test case from");return}try{const U=await Se(`/projects/${e.id}/eval-sets`);if(J1(U.eval_sets||[]),!U.eval_sets||U.eval_sets.length===0){confirm(`No evaluation sets found. Would you like to create one first?

Go to the Evals tab to create an evaluation set.`)&&(window.location.href=`/project/${e.id}/evals`);return}zp(U.eval_sets[0].id),ds(!0)}catch(U){alert(`Error loading eval sets: ${U.message||"Unknown error"}`)}},[f,e]),Y1=k.useCallback(async()=>{if(!f||!e||!Oi){alert("Please select an evaluation set");return}Ap(!0);try{const U=await Se(`/projects/${e.id}/session-to-eval-case`,{method:"POST",body:JSON.stringify({session_id:f,eval_set_id:Oi,case_name:_l})});ds(!1),alert(`Test case "${U.eval_case.name}" created successfully!

Token count: ${U.session_token_count.toLocaleString()} tokens

Go to the Evals tab to view and edit the test case.`)}catch(U){alert(`Error creating test case: ${U.message||"Unknown error"}`)}finally{Ap(!1)}},[f,e,Oi,_l]),Q1=k.useCallback(()=>{const U=document.createElement("input");U.type="file",U.accept=".json",U.onchange=async te=>{var me;const de=(me=te.target.files)==null?void 0:me[0];if(de)try{const pe=await de.text(),Ae=JSON.parse(pe);if(!Ae.events||!Array.isArray(Ae.events)){alert("Invalid run file: missing events array");return}a(),l(),y(null),T(null),Ae.events.forEach(Le=>{s(Le)})}catch(pe){alert(`Failed to load run file: ${pe}`)}},U.click()},[a,l,s]);return k.useEffect(()=>{if(x!==null){const U=document.querySelector(".event-row.selected");U==null||U.scrollIntoView({behavior:"smooth",block:"nearest"})}},[x]),e?o.jsxs("div",{className:`run-panel ${ge?"resizing":""}`,children:[o.jsx("style",{children:`
        .run-panel {
          display: flex;
          flex-direction: column;
          width: 100%;
