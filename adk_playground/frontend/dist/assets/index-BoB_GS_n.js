...
def my_callback(tool: BaseTool, tool_args: Dict[str, Any], tool_context: ToolContext, result: Dict) -> Optional[Dict]:
    ...
    return None
`;default:return Qx("before_agent")}}function _C({onSelectCallback:e}){var j,R;const{project:t,updateProject:n,addCustomCallback:r,updateCustomCallback:i,removeCustomCallback:o}=ut(),[a,l]=b.useState(""),[c,u]=b.useState(null),[d,p]=b.useState(null),[f,m]=b.useState(!1),y=b.useRef(null);if(!t)return null;const _=t.custom_callbacks||[],E=_.find(w=>w.id===c);function v(w){u(w),e==null||e(w)}b.useEffect(()=>{E?(l(E.code),p(null)):l("")},[c,E]);function x(){const w=jC(),L={id:w,name:"new_callback",description:"",module_path:"callbacks.custom",code:Qx("before_agent"),state_keys_used:[]};r(L),v(w)}function g(w,L){L.stopPropagation(),confirm("Delete this callback?")&&(o(w),c===w&&(e==null||e(null),u(null),l("")))}function P(){if(!c)return;const w=_.find(V=>V.id===c);if(!w)return;const L=w.name.trim();if(!L){alert("Please enter a name");return}if(!Uf(L)){p("Name must contain only alphanumeric characters and underscores");return}if(_.find(V=>V.name===L&&V.id!==w.id)){p("A callback with this name already exists");return}i(w.id,{code:a,name:L,description:w.description,module_path:w.module_path,state_keys_used:w.state_keys_used}),p(null)}function T(w,L){y.current=L;try{Gx(L,{endpoint:"/api/code-completion",language:"python"})}catch($){console.warn("Failed to register Monacopilot completion:",$)}}async function S(){if(E){m(!0);try{let w="before_agent";const L=E.name.toLowerCase(),$=E.description.toLowerCase();L.includes("after_agent")||$.includes("after agent")?w="after_agent":L.includes("before_model")||$.includes("before model")?w="before_model":L.includes("after_model")||$.includes("after model")?w="after_model":L.includes("before_tool")||$.includes("before tool")?w="before_tool":L.includes("after_tool")||$.includes("after tool")?w="after_tool":(L.includes("before_agent")||$.includes("before agent"))&&(w="before_agent");const V=await Mg(t.id,E.name,E.description,w,E.state_keys_used);V.success&&V.code?(l(V.code),i(E.id,{code:V.code})):(console.error("Failed to generate callback code:",V.error),alert("Failed to generate callback code: "+(V.error||"Unknown error")))}catch(w){console.error("Error generating callback code:",w),alert("Error generating callback code: "+w.message)}finally{m(!1)}}}const C=((R=(j=t.app)==null?void 0:j.state_keys)==null?void 0:R.map(w=>w.name))||[];return s.jsxs("div",{className:"tools-panel",children:[s.jsx("style",{children:`
        .tools-panel {
          display: flex;
          height: 100%;
          background: var(--bg-primary);
...
Go to the Evals tab to view and edit the test case.`)}catch(I){alert(`Error creating test case: ${I.message||"Unknown error"}`)}finally{Ju(!1)}},[f,e,as,pa]),uy=b.useCallback(()=>{const I=document.createElement("input");I.type="file",I.accept=".json",I.onchange=async ce=>{var he;const ae=(he=ce.target.files)==null?void 0:he[0];if(ae)try{const ke=await ae.text(),Oe=JSON.parse(ke);if(!Oe.events||!Array.isArray(Oe.events)){alert("Invalid run file: missing events array");return}a(),l(),g(null),T(null),Oe.events.forEach(pt=>{o(pt)})}catch(ke){alert(`Failed to load run file: ${ke}`)}},I.click()},[a,l,o]);return b.useEffect(()=>{if(x!==null){const I=document.querySelector(".event-row.selected");I==null||I.scrollIntoView({behavior:"smooth",block:"nearest"})}},[x]),e?s.jsxs("div",{className:`run-panel ${q?"resizing":""}`,children:[s.jsx("style",{children:`
        .run-panel {
          display: flex;
          flex-direction: column;
          width: 100%;
