...
def my_callback(tool: BaseTool, tool_args: Dict[str, Any], tool_context: ToolContext, result: Dict) -> Optional[Dict]:
    ...
    return None
`;default:return ax("before_agent")}}function kS({onSelectCallback:e}){var k,A;const{project:t,updateProject:n,addCustomCallback:r,updateCustomCallback:i,removeCustomCallback:o}=at(),[a,l]=w.useState(""),[c,u]=w.useState(null),[d,p]=w.useState(null),[f,m]=w.useState(!1),y=w.useRef(null);if(!t)return null;const b=t.custom_callbacks||[],T=b.find(S=>S.id===c);function x(S){u(S),e==null||e(S)}w.useEffect(()=>{T?(l(T.code),p(null)):l("")},[c,T]);function g(){const S=bS(),v={id:S,name:"new_callback",description:"",module_path:"callbacks.custom",code:ax("before_agent"),state_keys_used:[]};r(v),x(S)}function h(S,v){v.stopPropagation(),confirm("Delete this callback?")&&(o(S),c===S&&(e==null||e(null),u(null),l("")))}function P(){if(!c)return;const S=b.find(F=>F.id===c);if(!S)return;const v=S.name.trim();if(!v){alert("Please enter a name");return}if(!Xp(v)){p("Name must contain only alphanumeric characters and underscores");return}if(b.find(F=>F.name===v&&F.id!==S.id)){p("A callback with this name already exists");return}i(S.id,{code:a,name:v,description:S.description,module_path:S.module_path,state_keys_used:S.state_keys_used}),p(null)}function C(S,v){y.current=v;try{ox(v,{endpoint:"/api/code-completion",language:"python"})}catch(E){console.warn("Failed to register Monacopilot completion:",E)}}async function _(){if(T){m(!0);try{let S="before_agent";const v=T.name.toLowerCase(),E=T.description.toLowerCase();v.includes("after_agent")||E.includes("after agent")?S="after_agent":v.includes("before_model")||E.includes("before model")?S="before_model":v.includes("after_model")||E.includes("after model")?S="after_model":v.includes("before_tool")||E.includes("before tool")?S="before_tool":v.includes("after_tool")||E.includes("after tool")?S="after_tool":(v.includes("before_agent")||E.includes("before agent"))&&(S="before_agent");const F=await qh(t.id,T.name,T.description,S,T.state_keys_used);F.success&&F.code?(l(F.code),i(T.id,{code:F.code})):(console.error("Failed to generate callback code:",F.error),alert("Failed to generate callback code: "+(F.error||"Unknown error")))}catch(S){console.error("Error generating callback code:",S),alert("Error generating callback code: "+S.message)}finally{m(!1)}}}const N=((A=(k=t.app)==null?void 0:k.state_keys)==null?void 0:A.map(S=>S.name))||[];return s.jsxs("div",{className:"tools-panel",children:[s.jsx("style",{children:`
        .tools-panel {
          display: flex;
          height: 100%;
          background: var(--bg-primary);
