...
Go to the Evals tab to view and edit the test case.`)}catch(U){alert(`Error creating test case: ${U.message||"Unknown error"}`)}finally{_p(!1)}},[f,e,Mi,vl]),B1=j.useCallback(()=>{const U=document.createElement("input");U.type="file",U.accept=".json",U.onchange=async se=>{var xe;const he=(xe=se.target.files)==null?void 0:xe[0];if(he)try{const de=await he.text(),ze=JSON.parse(de);if(!ze.events||!Array.isArray(ze.events)){alert("Invalid run file: missing events array");return}a(),l(),y(null),A(null),ze.events.forEach(Pe=>{s(Pe)})}catch(de){alert(`Failed to load run file: ${de}`)}},U.click()},[a,l,s]);return j.useEffect(()=>{if(g!==null){const U=document.querySelector(".event-row.selected");U==null||U.scrollIntoView({behavior:"smooth",block:"nearest"})}},[g]),e?o.jsxs("div",{className:`run-panel ${K?"resizing":""}`,children:[o.jsx("style",{children:`
        .run-panel {
          display: flex;
          flex-direction: column;
          width: 100%;
