...
`;function fS({onSelectTool:e}){var Ce;const{project:t,updateProject:n,addCustomTool:r,updateCustomTool:i,removeCustomTool:o,selectedToolId:a,setSelectedToolId:l,builtinTools:c,mcpServers:u}=et(),[d,p]=_.useState(""),[f,m]=_.useState(null),[y,w]=_.useState("tools"),[E,g]=_.useState(null),[h,v]=_.useState(""),[L,S]=_.useState(!1),[j,x]=_.useState(null),[z,A]=_.useState(!1),[N,b]=_.useState(!1),[T,M]=_.useState(null),[U,D]=_.useState(null);if(!t)return null;const Q=t.mcp_servers||[],P=t.custom_tools.find(W=>W.id===a),O=Q.find(W=>W.name===E),I=new Set(Q.map(W=>W.name)),k=u.filter(W=>!I.has(W.name));function V(W){l(W),e==null||e(W)}_.useEffect(()=>{P&&(p(P.code),D(null))},[a]),_.useEffect(()=>{O&&(v(JSON.stringify(O,null,2)),S(!1))},[E]);function K(){const W=uS();r({id:W,name:"new_tool",description:"",module_path:"tools.custom",code:pS,state_keys_used:[]}),V(W)}function C(W,he){he.stopPropagation(),confirm("Delete this tool?")&&(o(W),a===W&&(e==null||e(null)))}function le(W){P&&(W.name!==void 0&&(W.name===""||dS(W.name)?D(null):D("Name can only contain letters, numbers, and underscores")),i(P.id,W))}function R(W){W!==void 0&&P&&(p(W),le({code:W}))}async function X(){if(P){A(!0);try{const W=await Wh(t.id,P.name,P.description,P.state_keys_used);W.success&&W.code?(p(W.code),le({code:W.code})):(console.error("Failed to generate tool code:",W.error),alert("Failed to generate tool code: "+(W.error||"Unknown error")))}catch(W){console.error("Error generating tool code:",W),alert("Error generating tool code: "+W.message)}finally{A(!1)}}}const B=_.useRef(null),ee=_.useCallback((W,he)=>{if(B.current&&typeof B.current=="function")try{B.current()}catch{}try{const Te=sx(he,W,{language:"python",endpoint:"/api/code-completion",trigger:"onTyping"});typeof Te=="function"?B.current=Te:B.current=null}catch{B.current=null}},[]);_.useEffect(()=>()=>{if(B.current&&typeof B.current=="function")try{B.current()}catch{}},[]);function oe(){const W={name:`mcp_server_${Date.now().toString(36)}`,description:"New MCP Server",connection_type:"stdio",command:"npx",args:["-y","@modelcontextprotocol/server-example"],env:{},headers:{},timeout:10};n({mcp_servers:[...Q,W]}),g(W.name),x(null)}function $(W){const he={...W};n({mcp_servers:[...Q,he]}),g(he.name),x(null)}function q(W,he){he.stopPropagation(),confirm("Delete this MCP server?")&&(n({mcp_servers:Q.filter(Te=>Te.name!==W)}),E===W&&g(null))}function J(W){W!==void 0&&(v(W),S(W!==JSON.stringify(O,null,2)))}function te(){if(O)try{const W=JSON.parse(h),he=O.name,Te=Q.map(mt=>mt.name===he?W:mt);n({mcp_servers:Te}),g(W.name),S(!1)}catch(W){alert("Invalid JSON: "+W.message)}}async function ae(){b(!0),M(null);try{const W=JSON.parse(h),he=await qh({connection_type:W.connection_type,command:W.command,args:W.args,env:W.env,url:W.url,headers:W.headers,timeout:W.timeout||30});if(M(he),he.success&&he.tools.length>0){const Te={...W,tool_filter:he.tools.map(mt=>mt.name)};v(JSON.stringify(Te,null,2)),S(!0)}}catch(W){M({success:!1,tools:[],error:W.message})}finally{b(!1)}}const de={};return t.custom_tools.forEach(W=>{const he=W.module_path||"tools";de[he]||(de[he]=[]),de[he].push(W)}),s.jsxs("div",{className:"tools-panel",children:[s.jsx("style",{children:`
        .tools-panel {
          display: flex;
          gap: 20px;
          height: calc(100vh - 180px);
