...
`).length,m=Math.max(r,Math.min(f*20+40,500));l(m)}},[e,r]),_.useEffect(()=>{i&&u.current&&u.current.focus()},[i]);function d(f){u.current=f,f.focus()}function p(f){c.current&&!c.current.contains(f.target)&&o(!1)}return _.useEffect(()=>{if(i)return document.addEventListener("mousedown",p),()=>document.removeEventListener("mousedown",p)},[i]),s.jsxs("div",{ref:c,className:"markdown-editor",children:[s.jsx("style",{children:`
      ...
      `}),i?s.jsx("div",{className:"monaco-wrapper",children:s.jsx(an,{height:"100%",language:"markdown",theme:"vs-dark",value:e,onChange:f=>t(f||""),onMount:d,options:{minimap:{enabled:!1},fontSize:14,fontFamily:"'JetBrains Mono', monospace",lineNumbers:"off",scrollBeyondLastLine:!1,automaticLayout:!0,wordWrap:"on",padding:{top:12,bottom:12},lineDecorationsWidth:8,folding:!1,glyphMargin:!1,renderLineHighlight:"none",overviewRulerLanes:0,hideCursorInOverviewRuler:!0,scrollbar:{vertical:"auto",horizontal:"hidden"}}})}):s.jsxs("div",{className:"markdown-preview",onClick:()=>o(!0),children:[s.jsxs("span",{className:"edit-hint",children:[s.jsx(x0,{size:12}),"Click to edit"]}),e?s.jsx("div",{className:"markdown-content",children:s.jsx(ex,{children:e})}):s.jsx("div",{className:"markdown-placeholder",children:n})]})]})}function S_(e){return/^[a-zA-Z0-9_]+$/.test(e)}function C_({agent:e}){const{project:t,updateAgent:n,mcpServers:r,builtinTools:i,setActiveTab:o,setRunAgentId:a}=et(),[l,c]=_.useState(new Set(["basic","model","tools","subagents"])),[u,d]=_.useState(!1),[p,f]=_.useState(!1),[m,y]=_.useState(!1),[w,E]=_.useState(""),[g,h]=_.useState(null);if(!t)return null;function v(P){n(e.id,P)}function L(P){if(P===""){h(null),v({name:P});return}S_(P)?h(null):h("Name can only contain letters, numbers, and underscores"),v({name:P})}function S(P){const M=new Set(l);M.has(P)?M.delete(P):M.add(P),c(M)}const k=e.type==="LlmAgent",x=e;function z(P){k&&v({tools:[...x.tools,P]})}function A(P){k&&v({tools:x.tools.filter((M,I)=>I!==P)})}function N(P,M){if(!k)return;const I=[...x.tools];I[P]=M,v({tools:I})}function b(P){"sub_agents"in e&&v({sub_agents:[...e.sub_agents,P]})}function T(P){"sub_agents"in e&&v({sub_agents:e.sub_agents.filter(M=>M!==P)})}const O=t.agents.filter(P=>P.id!==e.id);async function V(){if(!(!t||!k)){d(!0);try{const P=x.instruction||"",M=P?`Current instruction to improve:

${P}

Please improve and expand this instruction while preserving its core intent.`:void 0,j=t.agents.some(K=>K.id===e.id)?void 0:e,U=await Fs(t.id,e.id,M,j);U.success&&U.prompt?v({instruction:U.prompt}):alert("Failed to generate prompt: "+(U.error||"Unknown error"))}catch(P){alert("Error generating prompt: "+P.message)}finally{d(!1)}}}async function D(){if(!(!t||!k||!w.trim())){d(!0);try{const M=`Current instruction:
...
Please apply the requested changes to the instruction above. Output only the updated instruction, nothing else.`,j=t.agents.some(K=>K.id===e.id)?void 0:e,U=await Fs(t.id,e.id,M,j);U.success&&U.prompt?(v({instruction:U.prompt}),y(!1),E("")):alert("Failed to apply changes: "+(U.error||"Unknown error"))}catch(P){alert("Error applying changes: "+P.message)}finally{d(!1)}}}async function Q(){if(!t||!k)return;const P=x.instruction;if(!P){alert("Please write an instruction first");return}f(!0);try{const M=`Write a 5-10 word description of what this agent does.
